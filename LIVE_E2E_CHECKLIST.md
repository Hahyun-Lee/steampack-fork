...
Except for the quit row, compare all four surfaces: the menu-bar icon/text, `pmset -g` (`SleepDisabled`), the local `applied-state.json`, and both visible Control Center tiles. After quit, check process absence, restored `pmset`, a missing applied-state record, and unavailable Control Center providers.
...
| App/menu change while Control Center is closed → open immediately | Changed tiles converge through the immediate targeted reload plus at most one targeted retry after one second; they do not wait for the ten-second runtime refresh. |
...
| Running any mode → Quit & Restore Sleep | Normal sleep is restored, the applied-state record is removed, and within 6 seconds both controls report unavailable—not OFF. Using either control instructs the user to open SteamPack. |
