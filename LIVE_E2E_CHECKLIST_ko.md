...
종료 행을 제외하고 메뉴 막대의 SteamPack 상태 아이콘·문구, `pmset -g`의 `SleepDisabled`, 로컬 `applied-state.json`, 화면의 제어 센터 타일 두 개를 모두 대조합니다. SteamPack 제어 항목을 메뉴 막대에 고정했다면 고정된 항목과 펼친 패널의 같은 항목도 대조합니다. 종료 행에서는 프로세스가 사라졌는지, `pmset`이 복구됐는지, applied-state 기록이 만료·무효화됐는지, 제어 센터 provider가 확인 불가 상태를 보고하는지 확인합니다.
...
| 정상 → 덮개 닫고 계속 ON → 0.5초 안에 덮개 닫기 OFF | 잠자기 방지는 ON으로 남아야 합니다. 두 번째 동작이 들어와도 첫 번째 동작이 예약한 잠자기 방지 타일 재요청은 취소되지 않아야 하며, 화면의 두 타일이 1초 안에 실제 상태와 일치해야 합니다. |
| 제어 센터를 닫은 채 SteamPack 상태 메뉴에서 덮개 닫기 ON → 즉시 제어 센터 열기 | 1초 안에 SteamPack 상태 아이콘, applied state, 눈 타일, 노트북 타일이 모두 ON/ON이어야 합니다. 로컬 상태가 ON/ON인데 반쯤 감긴 눈이나 빗금 노트북이 보이면 배포 차단입니다. 즉시 요청과 0.5초 뒤 한 번뿐인 대상별 재요청 안에 수렴해야 하며, 10초 runtime refresh를 기다리면 실패입니다. |
| 덮개 닫기 OFF → 잠자기 방지 OFF | 첫 동작 뒤에는 눈은 ON, 노트북은 OFF이고 두 번째 동작 뒤에는 모두 OFF여야 합니다. 각 동작 뒤 1초 안에 상태 아이콘, 고정 항목, 펼친 패널 타일이 일치해야 합니다. |
...
| 임의 모드 실행 중 → 종료하고 정상 잠자기로 복구 | 정상 잠자기를 복구하고 applied-state 기록을 무효화합니다. 6초 안에 두 컨트롤은 OFF가 아니라 확인 불가 상태가 되며, 누르면 SteamPack을 열라는 안내가 나옵니다. |
