...
**덮개 닫고 계속**을 켜면 **잠자기 방지**도 함께 켜집니다. **잠자기 방지**를 끄면 정상 잠자기 동작으로 돌아갑니다.
...
권한 파일은 현재 계정의 숫자 사용자 ID별로 분리됩니다. **덮개 닫기 권한 설치…**는 이전 공용 규칙이 현재 계정의 규칙과 정확히 일치할 때만 안전하게 이전하며, **덮개 닫기 권한 제거…**는 다른 계정의 규칙을 삭제하지 않습니다.
...
SteamPack 앱은 실행 중이어야 합니다. 앱이 꺼져 있으면 컨트롤은 OFF를 표시하며, 컨트롤을 사용하면 SteamPack을 열라는 안내가 나타납니다.
...
제어 센터 provider는 macOS에 실제로 적용된 상태를 읽습니다. 적용 상태 기록이 없거나 만료되면 provider는 OFF로 판단하고, build 7은 컨트롤별 갱신과 전체 제어 센터 갱신을 모두 요청합니다. 다만 macOS가 화면의 타일 캐시를 유지할 수 있으므로, 실제 타일이 OFF로 수렴하는지는 바이너리 배포 전에 [설치 앱 점검표](LIVE_E2E_CHECKLIST_ko.md)를 통과해야 합니다.
...
서명된 빌드를 배포하기 전에는 [설치 앱·제어 센터 실기기 점검표](LIVE_E2E_CHECKLIST_ko.md)를 완료해야 합니다. 단위 테스트로 볼 수 없는 설치 앱·내장 확장 버전과 화면의 타일 전환을 확인합니다.
